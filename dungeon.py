# create the dungeon
# this code is from https://github.com/mizipzor/roguelike-dungeon-generator/blob/master/dungeon.py
...
def generate(cellsX, cellsY, cellSize=5):
    ...
    value_list  = list(cells.values())
    current = lastCell = firstCell = random.choice(value_list)
    ...
    while True:
        ...
        unconnected = [lambda x: not x.connected, getNeighborCells(current)]
        if not unconnected:
            break

        # 3a. Connect to one of them.
        unconnected_list = list(unconnected)
        print('unconn:' ,unconnected)
        print('list: ',unconnected_list)
        neighbor = random.choice(unconnected)
        #neighbor = unconnected[3]
    ...
    # create tiles
    tiles = {}
    tilesX = cellsX * 4 #cellSize
    tilesY = cellsY * 4 #cellSize
